...
public class ComparatorUtil {
    ...
    public static boolean equals(Object a, Object b, Method getter) {
        ...
        boolean result;
        if (a instanceof List) {
            return listEquals((List) a, (List) b, getter);
        } else {
    ...
    private static <T extends Comparable<? super T>> boolean listEquals(List<T> a, List<T> b, Method getter) {
        LOG.info("{} lists verification:", getter.getName());
        a = (List<T>) ReflectionUtil.invokeMethod(a, getter);
        b = (List<T>) ReflectionUtil.invokeMethod(b, getter);
